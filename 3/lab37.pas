...
PROCEDURE ObhodVerhNiz(T: ukaz); {обход сверху вниз}
VAR
  Top, Kon: point;
  K: ukaz;  
...
BEGIN
  IF t <> NIL THEN
    BEGIN
      ...
      p := t;
      For j := 1 to t^.urov 
      DO  { отступ в зависимости от уровня }
        BEGIN
          p := p^.fath;
          St := '.' + St
        END;
...
BEGIN
  ...
  DO
    BEGIN
      stop := true;
      assign(Fin,'C:\Users\s1200501047\asd\input.txt');
      assign(Fout,'C:\Users\s1200501047\asd\output.txt');
      Reset (Fin);
      Rewrite (Fout);
