    ...
    BEGIN
      ...
      For j := 1 to t^.urov DO  { отступ в зависимости от уровня }
        BEGIN
          ...
          St := '.' + St
        END;
      IF (t^.fath = NIL) AND (t^.minWeight > max)
      THEN
        WRITELN(F, 'Tree is empty. Min in input tree is ', t^.minWeight);
      ...
      THEN
        BEGIN
          ...
          THEN
            ...
            END
          ELSE
            WRITELN(Fout, 'File don''t have the tree. Max = ', max)
        END
      ELSE
        WRITELN(Fout, 'File is empty');
